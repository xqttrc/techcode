...
Ext.define("Easyweb.History", {
    ...
    navigate: function(token, noHistory) {
        ...
        if (url.url == "#!/api") {
            Easyweb.App.getController('Classes').loadIndex(noHistory);
        }
        else if (url.type === "api") {
            Easyweb.App.getController('Classes').loadClass(url.url, noHistory);
        }
        else if (url.url === "#!/guide") {
            Easyweb.App.getController('Guides').loadIndex(noHistory);
        }
        else if (url.type === "guide") {
            Easyweb.App.getController('Guides').loadGuide(url.url, noHistory);
        }
        else if (url.url === "#!/video") {
            Easyweb.App.getController('Videos').loadIndex(noHistory);
        }
        else if (url.type === "video") {
            Easyweb.App.getController('Videos').loadVideo(url.url, noHistory);
        }
        else if (url.url === "#!/example") {
            Easyweb.App.getController('Examples').loadIndex();
        }
        else if (url.type === "example") {
            Easyweb.App.getController('Examples').loadExample(url.url, noHistory);
        }
        else if (url.url === "#!/comment") {
            Easyweb.App.getController('Comments').loadIndex();
        }
        else if (url.url === "#!/tests") {
            Easyweb.App.getController('Tests').loadIndex();
        }
        else {
            if (Easyweb.App.getController('Welcome').isActive()) {
                Easyweb.App.getController('Welcome').loadIndex(noHistory);
            }
