 ...
 */
Ext.define('Easyweb.controller.Classes', {
    extend: 'Easyweb.controller.Content',
    baseUrl: '#!/api',
    ...
    loadClass: function(url, noHistory) {
        ...
        noHistory || Easyweb.History.push(url);

        // separate class and member name
        var matches = url.match(/^#!\/api\/(.*?)(?:-(.*))?$/);
        var cls = Easyweb.ClassRegistry.canonicalName(matches[1]);
