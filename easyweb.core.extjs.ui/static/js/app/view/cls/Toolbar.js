 ...
 */
Ext.define('Easyweb.view.cls.Toolbar', {
    extend: 'Ext.toolbar.Toolbar',
    requires: [
        'Easyweb.view.HoverMenuButton',
        'Easyweb.Settings',
        'Easyweb.Comments',
    ...
    initComponent: function() {
        ...
        Ext.Array.forEach(Easyweb.data.memberTypes, function(type) {
    ...
    createCb: function(text, type) {
        return Ext.widget('menucheckitem', {
            text: text,
            checked: Easyweb.Settings.get("show")[type],
            listeners: {
    ...
    createMemberButton: function(cfg) {
        ...
        return Ext.create('Easyweb.view.HoverMenuButton', {
            text: cfg.text,
            cls: 'icon-'+cfg.type,
    ...
    createLinkRecord: function(cls, member) {
        return {
            ...
            meta: member.meta,
            commentCount: Easyweb.Comments.getCount(["class", cls, member.id])
        };
    },
    ...
    refreshMenuCommentCounts: function() {
        Ext.Object.each(this.memberButtons, function(key, btn) {
            btn.getStore().each(function(r) {
                r.set("commentCount", Easyweb.Comments.getCount(["class", this.docClass.name, r.get("id")]));
            }, this);
        }, this);
    }
});
