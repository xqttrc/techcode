 ...
 */
Ext.define("Easyweb.GuideSearch", {
    singleton: true,
     ...
     */
    isEnabled: function() {
        return !!Easyweb.data.guideSearch.url;
    },
    ...
    search: function(term, callback, scope) {
        var request = this.currentRequest = Ext.data.JsonP.request({
            url: Easyweb.data.guideSearch.url,
            params: {
                fragsize: 32,
                max_fragments: 1,
                q: term,
                product: Easyweb.data.guideSearch.product,
                version: Easyweb.data.guideSearch.version,
                start: 0,
