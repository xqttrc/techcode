Ext.define("easyweb.core.base.LoginForm", {
    ...
    _buildItems_: function() {
        return [{
                xtype: "textfield",
                itemId: "usr",
                fieldLabel: Easyweb.translate("msg", "login_user"),
                ...
                xtype: "textfield",
                itemId: "pswd",
                fieldLabel: Easyweb.translate("msg", "login_pswd"),
                inputType: "password",
                listeners: {
                    change: {
                        scope: this,
                ...
                xtype: "textfield",
                itemId: "client",
                fieldLabel: Easyweb.translate("msg", "login_client"),
            ...
            /*, {
             xtype : "combo",
             itemId : "lang",
             fieldLabel : Easyweb.translate("msg", "login_lang"),
             forceSelection : true,
             triggerAction : "all",
    ...
    onActionSuccess: function(response, options) {
        ...
        if (r.data.extjsDateFormat) {
            Easyweb.DATE_FORMAT = r.data.extjsDateFormat;
        }
        if (r.data.extjsTimeFormat) {
            Easyweb.TIME_FORMAT = r.data.extjsTimeFormat;
        }
        if (r.data.extjsDateTimeFormat) {
            Easyweb.DATETIME_FORMAT = r.data.extjsDateTimeFormat;
        }

        if (r.data.extjsAltFormats) {
            Easyweb.DATE_ALTFORMATS = r.data.extjsAltFormats;
        }

        if (r.data.decimalSeparator) {
            Easyweb.DECIMAL_SEP = r.data.decimalSeparator;
        }
        if (r.data.thousandSeparator) {
            Easyweb.THOUSAND_SEP = r.data.thousandSeparator;
        }
        Easyweb.initFormats();
    ...
    doLogin: function(btn, evnt) {
        ...
        Ext.Ajax.request({
            ...
            scope: this,
            url: Easyweb.sessionAPI("json").login,
            timeout: 600000
        });
...
Ext.define("easyweb.core.base.LoginWindow", {
    extend: "Ext.Window",
    initComponent: function(config) {

        var btn = Ext.create('Ext.Button', {
            text: Easyweb.translate("msg", "login_btn"),
            disabled: true
        });

        var cfg = {
            title: Easyweb.translate("msg", "login_title"),
            border: true,
            width: 350,
            resizable: false,
            closeAction: "hide",
            ...
            buttonAlign: "center",
            modal: true,
            items: new easyweb.core.base.LoginForm({
                actionButton: btn
            }),
            buttons: [btn]
        };
